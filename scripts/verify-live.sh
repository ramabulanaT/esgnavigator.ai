...
[[ -z "$API_URL" ]] && { err "Cannot determine API_URL. Run: bash scripts/verify-live.sh https://<app>.up.railway.app"; exit 2; }
...
[[ "$code" =~ ^(2|3)[0-9][0-9]$ ]] && ok "Web reachable ($code)" || { err "Web HTTP $code @ $WEB_URL"; exit 1; }
...
[[ "$health_ok" -eq 1 ]] || { err "No working health at $API_URL"; printf 'Hint: implement GET /health → {\"ok\":true}\n'; exit 1; }

# Sync via uptime
ujson="$(curl -fsS "$WEB_URL/api/internal/uptime" || true)"
if [[ -z "$ujson" ]]; then
  warn "No /api/internal/uptime (non-fatal)."
else
  ...
  if [[ -n "$api_from_uptime" ]]; then
    if [[ "${api_from_uptime%/}" == "${API_URL%/}" ]]; then ok "NEXT_PUBLIC_API_URL matches Railway"
    else
